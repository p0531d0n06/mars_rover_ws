...
Top-level launch file — starts the complete Mars rover simulation:
  ...
  8. LiDAR view RViz (rover model + 3D point cloud)
...
def generate_launch_description():
    ...
    # ── 4. LiDAR view RViz (rover model + 3D point cloud) ─────────────
    lidar_view_launch = TimerAction(
        period=3.0,
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(pkg_gazebo, 'launch', 'lidar_view.launch.py')
            ),
            launch_arguments={'use_sim_time': use_sim_time}.items(),
        )],
    )
