...
class ArucoWaypointBridge(Node):
    def __init__(self):
        ...
        # Nav2 approach positions — 2.5 m in front of each marker facing toward it.
        # Computed as: marker_pos - unit(marker_pos) × 2.5 m
        self._waypoints = {
            0: {'x':  7.5, 'y':  0.0, 'yaw_deg':   0.0, 'name': 'Alpha'},    # marker (10, 0)
            1: {'x':  8.2, 'y':  8.2, 'yaw_deg':  45.0, 'name': 'Beta'},     # marker (10, 10)
            2: {'x':  0.0, 'y': 13.5, 'yaw_deg':  90.0, 'name': 'Gamma'},    # marker (0, 16)
            3: {'x': -8.2, 'y':  8.2, 'yaw_deg': 135.0, 'name': 'Delta'},    # marker (-10, 10)
            4: {'x': -8.2, 'y': -8.2, 'yaw_deg': 225.0, 'name': 'Epsilon'},  # marker (-10, -10)
            5: {'x':  8.2, 'y': -8.2, 'yaw_deg': 315.0, 'name': 'Zeta'},     # marker (10, -10)
            6: {'x':  0.0, 'y':-13.5, 'yaw_deg': 270.0, 'name': 'Eta'},      # marker (0, -16)
            7: {'x': 13.5, 'y':  0.0, 'yaw_deg':   0.0, 'name': 'Theta'},    # marker (16, 0)
            8: {'x':-13.5, 'y':  0.0, 'yaw_deg': 180.0, 'name': 'Iota'},     # marker (-16, 0)
            9: {'x':  4.2, 'y':  4.2, 'yaw_deg':  45.0, 'name': 'Kappa'},    # marker (6, 6)
        }
    ...
    def _navigate_next(self):
        ...
        if not self._nav2_client.wait_for_server(timeout_sec=0.0):
            self.get_logger().warn('Nav2 not ready — retrying in 5 s.')
    ...
    def _control_loop(self):
        ...
        if self.state == State.SCANNING:
            ...
            # Expire stale detections (camera at 15 Hz → fresh within 1 s)
            age_s = (self.get_clock().now().nanoseconds - info.get('_stamp_ns', 0)) / 1e9
            if age_s > 1.0:
                self.get_logger().warn(
                    f'Detection for marker {self._active_id} stale ({age_s:.1f} s) — declaring arrived.')
                self._latest_poses.pop(self._active_id, None)
                self._stop()
                self._declare_arrived()
                return
    ...
    def _return_to_start(self):
        ...
        if not self._nav2_client.wait_for_server(timeout_sec=0.0):
            self.get_logger().error('Nav2 not ready for return — done.')
...
"""
    ...
    def _make_pose_stamped(self, x, y, yaw_deg):
        ps = PoseStamped()
        ps.header.stamp    = RosTime()   # sec=0, nanosec=0 → use latest available TF
